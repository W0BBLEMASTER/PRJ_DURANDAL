...
// LIBRETRO LOGGING HOOK
extern "C" {
    void (*libretro_log_hook)(int level, const char *fmt, ...) = NULL;
}
...
TopLevelLogger::logMessageV(const char* inDomain, int inLevel, const char* inFile, int inLine, const char* inMessage, va_list inArgs) {
    if(inLevel < sLoggingThreshhold) {
        ...
        if (libretro_log_hook) {
            libretro_log_hook(2, "[ALEPH] %s\n", stringBuffer); // 2 = INFO
        }

        fprintf(stderr, "[ALEPH] %s\n", stringBuffer);
        
        if (sOutputFile != NULL) {
            fprintf(sOutputFile, "%s\n", stringBuffer);
            if(sFlushOutput) fflush(sOutputFile);
        }
    }
}
