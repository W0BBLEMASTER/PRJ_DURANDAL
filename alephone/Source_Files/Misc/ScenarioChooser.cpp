...
{
    A1_TRACE("ScenarioChooser::run: ENTER");
	std::sort(scenarios_.begin(), scenarios_.end());

    extern void (*libretro_log_hook)(int level, const char *fmt, ...);
    if (libretro_log_hook) libretro_log_hook(1, "[PRJ_WRKN] ScenarioChooser::run() - Found %d scenarios.\n", (int)scenarios_.size());

    // LIBRETRO FIX: Use fixed width/height instead of desktop mode
	window_width_ = 640;
	window_height_ = 480;
	...
	{
		optimize_image(scenario, nullptr);
	}

	done_ = false;
	while (!done_)
	{
		libretro_yield(); // KEEP RETROARCH ALIVE
		SDL_Event e;
		...
		redraw(nullptr); // Pass nullptr, we'll use MainScreenSurface inside
		SDL_Delay(10);
	}
...
{
	auto format = MainScreenSurface()->format;
...
{
	auto surface = MainScreenSurface();
    if (!surface) return;
