...
{
    A1_TRACE("initialize_application: ENTER");
#if defined(__WIN32__)
	...
	// Initialize SDL
    A1_TRACE("initialize_application: SDL_Init");
    ...
    A1_TRACE("initialize_application: IMG_Init");
    ...
    A1_TRACE("initialize_application: joystick init");
	initialize_joystick();

	const string default_data_env = a1_getenv("ALEPHONE_DEFAULT_DATA");
#ifndef SCENARIO_IS_BUNDLED
    A1_TRACE("initialize_application: Scenario search start");
	...
	if (shell_options.directory != "")
	{
        A1_TRACE("initialize_application: Search path - setting option dir");
	...
	{
        A1_TRACE("initialize_application: Search path - setting env dir");
...
//	initialize_fonts();
    ...
    A1_TRACE("initialize_application: Engine subsystems starting");
	initialize_marathon();
    ...
    A1_TRACE("initialize_application: EXIT (SUCCESS)");
}
...
extern "C" void retro_loop_tick(void)
{
    ...
    if (game_state == _quit_game) {
        A1_TRACE("retro_loop_tick: QUIT state detected");
    }
}
